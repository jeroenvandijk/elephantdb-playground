...
public class NewDomainSpec extends DomainSpec {
     ...
     * @param numShards
     */
    public NewDomainSpec(String factClass, String shardSchemeClass, int numShards) {
        this(factClass, shardSchemeClass, numShards, new Args());
    }

    public NewDomainSpec(String factClass, String shardSchemeClass, int numShards, Args args) {
        this(Utils.classForName(factClass), Utils.classForName(shardSchemeClass), numShards, args);
    }

    public NewDomainSpec(Class factClass, Class shardSchemeClass, int numShards) {
        this(factClass, shardSchemeClass, numShards, new Args());
    }

    public NewDomainSpec(Class factClass, Class shardSchemeClass, int numShards, Args args) {
        this((Coordinator) Utils.newInstance(factClass),
                (ShardingScheme)Utils.newInstance(shardSchemeClass),
                numShards, args);
    }

    public NewDomainSpec(Coordinator coordinator, ShardingScheme shardingScheme, int numShards) {
        this(coordinator, shardingScheme, numShards, new Args());
    }

    public NewDomainSpec(Coordinator coordinator, ShardingScheme shardingScheme, int numShards, Args args) {
        if (numShards <= 0) {
    ...
    @Override public boolean equals(Object obj) {
        ...
        NewDomainSpec o = (NewDomainSpec) obj;
    ...
    public static NewDomainSpec readFromFileSystem(FileSystem fs, String dirpath) throws IOException {
        ...
        NewDomainSpec ret = parseFromStream(is);
        is.close();
        return ret;
    }
    ...
    public static NewDomainSpec parseFromStream(InputStream is) {
        Map format = (Map) YAML.load(new InputStreamReader(is));
        return parseFromMap(format);
    }

    @SuppressWarnings("unchecked")
    protected static NewDomainSpec parseFromMap(Map<String, Object> specmap) {
        ...
        return new NewDomainSpec(persistenceConf, shardSchemeConf, numShards, args);
    }
    ...
    public void readFields(DataInput di) throws IOException {
        NewDomainSpec spec = parseFromMap((Map<String, Object>) YAML.load(WritableUtils.readString(di)));
